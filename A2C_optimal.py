...
class A2C(BaseAgent):
    ...
    def processInfo(self, res, done):
        ...
        for i in reversed(range(len(res)-1)):
            ...
            out[i] = ps[i:i+1], vs[i:i+1], returns, advantage
            actions[i] = a
    ...
    def train_A2C(self):
        ...
        for i_episode in range(self.n_episode):
            ...
            while not done:
                ...
                res.append([r, c, reward, a])
                r, c = nr, nc
                dis += 1
                if dis>self.max_dis:
                    break
                
            res.append([r, c, None,  None])
            (p, v, returns, advantage), a = self.processInfo(res, done)

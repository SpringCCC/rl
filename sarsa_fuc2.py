...
class SARSA_FA_Env(Env):
    def __init__(self, *args, **kwargs):
        ...
        self.q_net = QNetwork(self.m, self.n, self.n_actions)
        ...
        for ep in range(self.n_episode):
            ...
            while not done:
                ...
                if done:
                    ...
                    q_next = torch.max(self.q_net(s_next)).detach()
                    target = torch.tensor([r + self.gamma * q_next], dtype=torch.float32).cuda()

                # Q(s,a)
                q_sa = self.q_net(s)[0, a].cuda()

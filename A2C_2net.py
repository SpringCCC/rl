...
class A2C(BaseAgent):
    ...
    def train_A2C(self):
        for i_episode in range(self.n_episode):
            ...
            p = self.policy_net(r, c)
            probs = F.softmax(p, dim=-1)
            log_prob = F.log_softmax(p, dim=-1)
            log_prob_a = log_prob.gather(1, a)
            
            policy_loss_ = (- log_prob_a * advantage).mean()
            entropy_loss = (probs * log_prob).sum(dim=-1).mean()

            policy_loss = policy_loss_ + self.entropy_loss_w * entropy_loss
